...
let affiche_solution s m =
  ...
  for j = 7 downto 0 do for i = 0 to 7 do
...
let enregistre (p,x1,x2,x3,x4,x5,maxi,maxj) =
  ...
  for j = 0 to maxj-1 do
    ...
    for k = 7 downto 1 do t.(k) <- t.(k-1) done; t.(0) <- 0
